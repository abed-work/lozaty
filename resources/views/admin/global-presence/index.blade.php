...
@section('body')
    ...
    <script>
        ...
        function countries_table(countries) {
            let table = '';
            for (let i = 0; i < countries.length; i++) {
                let value = countries[i];
                table += '<tr id='+value['code']+'>';
                for (let key in value) {
                    table += '<td>' + value[key] + '</td>';
                }
                table+= '<td> <input type="checkbox" name="countries[]" value='+value['code'] +' /> </td>'
                table += '</tr>';
            }
            document.getElementById('table-countries').innerHTML = table;
        }
